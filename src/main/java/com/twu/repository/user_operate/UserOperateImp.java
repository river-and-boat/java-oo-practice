...
public class UserOperateImp implements IUserOperate {
    ...
    private boolean setDegreeIfInNatural(HotSearchModel hotSearchModel, Double price, Integer degree)
        ...
        if (hotPurchaseEvent != null) {
            ...
            if (price > hotPurchaseEvent.getPurchasePrice()) {
                ...
                // 怎么保证一致性还未找到解决方案
                return setDegreeIfInNaturalTool(hotSearchModel, price, degree);
            } else {
                // 钱不够，购买失败
                return false;
            }
    ...
    /**
     *
     * @param hotSearchModel 热搜
     * @param price 价钱
     * @param degree 热搜等级
     * @return 是否设置成功
     * @throws UserException 异常
     */
    private boolean setDegreeIfInNaturalTool(HotSearchModel hotSearchModel, Double price, Integer degree)
            throws UserException {
     ...
     * @return 是否购买成功
     */
    private boolean setDegreeIfNotInNatural(HotSearchModel hotSearchModel,
